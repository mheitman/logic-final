...
fun alpha [c: ChainingSystem]: HashTable {
	{h: HashTable | {
...
PutOK: check {
	all c,c': ChainingSystem, kv : KVPair, h,h': HashTable |
		chaining/put [c, c', kv] and h = alpha [c]  and h' = alpha [c'] => absHT/put [h, h', kv]
}

// This check should not produce a counterexample
DeleteOK: check {
	all c,c': ChainingSystem, k : Key, h,h': HashTable |
		chaining/delete [c, c', k] and h = alpha [c]  and h' = alpha [c'] => absHT/delete [h, h', k]
}

// This check should not produce a counterexample
LookupOK: check {
	all c: ChainingSystem, k : Key, v : Value, h: HashTable |
